...
> NOTE: 本节的内容参见: "Resolving-Parse-Tree–Abstract-Syntax-Mismatch-with-Inherited-Attributes"
...
The SDD in Fig. 5.4 computes terms like `3 * 5` and `3 * 5 * 7`. The **top-down** parse of input `3 * 5` begins with the production $T \to F T'$. Here, `F` generates the digit 3, but the operator `*` is generated by `T'`. Thus, the left operand 3 appears in a different subtree of the parse tree from `*`. An **inherited attribute** will therefore be used to pass the operand to the operator. 
...
翻译: 本例所用的文法节选自常见表达式文法的非左递归版本。在第 4.4 节介绍自顶向下语法分析时，我们就曾以这类文法作为贯穿示例。
