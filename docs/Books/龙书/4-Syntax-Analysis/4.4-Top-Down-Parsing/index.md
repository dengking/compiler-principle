...
> NOTE: grammar中，越是底层越是不可分，上述的`F`就是这样的一个例子
...
> 翻译: 在**自顶向下语法分析**的每一步中，核心问题都是确定应当对某个**非终结符**（例如 `A`）应用哪条**产生式**。一旦选定了一条 A 产生式，后续的分析过程就只是将该**产生式体**中的**终结符**与输入符号串逐一进行匹配。
...
> NOTE: 上面这段话需要结合 figure 4.12 进行阅读: "the first $E^{'}$ node"、"the second $E^{'}$ node" 都是指的 figure 4.12 中的节点
