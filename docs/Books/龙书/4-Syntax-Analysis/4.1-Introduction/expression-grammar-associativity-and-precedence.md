...
$$
\underbrace{E \rightarrow E + T}_{\text{最上层：} +} \qquad \underbrace{T \rightarrow T * F}_{\text{中间层：} *} \qquad \underbrace{F \rightarrow (E) \mid \mathbf{id}}_{\text{最底层：原子}}
$$
...
**为什么不可能生成"`2 + 3` 先算"的树？** 因为要让 `+` 的操作数是 `2 + 3`，就需要一个 `T` 能展开出 `2 + 3`——但 `T` 的产生式（`T → T * F | F`）里**根本没有 `+`**！文法从语法上就**堵死了**这条路。这正是"乘法优先级高于加法"被强制的机制。
